program test_VHC
    ...
    case (3)
        Ice = Construct_Type_Ice_EXP(Lf, EXP_phi, Tf, EXP_a, nsize)
    end select
    ...
    case (3)
        VHC = Type_VolumetricHeatCapacity_3Phase(Cp_soil, Cp_water, Cp_ice, nsize)
    case (4)
    ...
    type is (Type_VolumetricHeatCapacity_3Phase)
        ...
        type is (Type_Ice_GCC)
            ...
            do i = 1, nsize
                ...
                l_Ca = v_p%Calculate_Ca(phi1=1.0d0 - phi, phi2=phi - i_ps%Calculate_Ice(T(i)), phi3=i_ps%Calculate_Ice(T(i)), structure_Ice=Ice, rho_ice=Den_ice, Temperature=T(i))
                write (10, *), T(i), l_Cp, l_Ca
            end do
