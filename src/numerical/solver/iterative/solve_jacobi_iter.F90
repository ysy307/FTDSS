!> Jacobi iterative solver (stationary method).
!> x_{k+1} = x_k + M^{-1} (b - A x_k) where M = diag(A).
submodule(numerical_solver_interface) impl_solve_type_solver_jacobi_iter
    implicit none
contains
    ...
    module subroutine solve_type_solver_jacobi_iter(self, A, b, x)
        ...
        do iter = 1, self%max_iterations
            ...
            ! z = M^{-1} r (preconditioner acts as Jacobi smoother)
...
end submodule impl_solve_type_solver_jacobi_iter
