!> BiCGSafe solver — breakdown-free BiCGSTAB variant.
!> Uses a safe update strategy to avoid rho=0 breakdown.
!> Reference: Sleijpen & Fokkema, modified for safety.
submodule(numerical_solver_interface) impl_solve_type_solver_bicgsafe
...
contains
    ...
    !> BiCGSafe: BiCGSTAB with breakdown avoidance.
    !> If rho becomes too small, re-select shadow residual r0.
    module subroutine solve_type_solver_bicgsafe(self, A, b, x)
        ...
        do iter = 1, self%max_iterations
            ...
            ! Breakdown avoidance: if rho is too small, reset shadow residual
...
end submodule impl_solve_type_solver_bicgsafe
