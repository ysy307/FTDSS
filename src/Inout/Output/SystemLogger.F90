...
contains
    module subroutine Output_SystemLog(self, time)
        ...
        write (num_unit, '(a,f8.4,a)') "RSS Memory Usage: ", rss_mb, " MB"
        write (num_unit, '(a)') "----------------------------------------------"
        write (num_unit, '(a)') time%start%label//" Time: "//time%start%date(1:4)//"-"//time%start%date(5:6)//"-"//time%start%date(7:8)//"T"//time%start%time(1:2)//":"//time%start%time(3:4)//":"//time%start%time(5:6)//trim(time%start%zone)
        write (num_unit, '(a)') time%end%label//" Time: "//time%end%date(1:4)//"-"//time%end%date(5:6)//"-"//time%end%date(7:8)//"T"//time%end%time(1:2)//":"//time%end%time(3:4)//":"//time%end%time(5:6)//trim(time%end%zone)
        write (num_unit, '(a)') "----------------------------------------------"
