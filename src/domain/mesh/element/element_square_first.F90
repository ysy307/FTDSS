...
contains
    ...
    module function construct_square_first(input) result(element)
        ...
        ! Get element properties from the vtk_constants helper using the cell name
        ...
        ! Initialize the object with properties common to this element type
    ...
    !----------------------------------------------------------------------
    pure module function jacobian_square_first(self, r, node_coords) result(jac)
        implicit none
        class(type_square_first), intent(in) :: self
        type(type_dp_vector_3d), intent(in) :: r
        real(real64), intent(in) :: node_coords(:, :)
    ...
    !----------------------------------------------------------------------
    pure module function jacobian_det_square_first(self, r, node_coords) result(det_j)
        implicit none
        class(type_square_first), intent(in) :: self
        type(type_dp_vector_3d), intent(in) :: r
        real(real64), intent(in) :: node_coords(:, :)
    ...
    module subroutine is_in_square_first(self, cartesian, normalized, node_coords, is_in)
        ...
        do iter = 1, max_iter
            ...
            j21 = jac(2, 1)
            j22 = jac(2, 2)
            r%x = r%x + (j22 * dx - j12 * dy) / det_j
            r%y = r%y + (-j21 * dx + j11 * dy) / det_j
        end do
