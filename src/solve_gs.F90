!> Gauss-Seidel iterative solver (stationary method).
!> Uses preconditioner as the Gauss-Seidel sweep.
!> x_{k+1} = x_k + M^{-1} (b - A x_k)
submodule(numerical_solver_interface) impl_solve_type_solver_gs
    implicit none
contains
...
end submodule impl_solve_type_solver_gs
