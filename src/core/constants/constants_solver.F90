!> Defines core constants used throughout the application.
module core_constants_solver
    use, intrinsic :: iso_fortran_env, only: int32
    use :: stdlib_strings, only:strip
    use :: stdlib_ascii, only:to_lower
    use :: core_constants_base, only:type_constant_id, type_constant_value
    implicit none
    private

    ! ==========================================================
    ! Matrix Storage Types
    ! ==========================================================
    !> Dense matrix storage format
    integer(int32), parameter, public :: MATRIX_DENSE = 1
    !> Compressed Row Storage format
    integer(int32), parameter, public :: MATRIX_CSR = 2
    !> Coordinate list format (COO)
    integer(int32), parameter, public :: MATRIX_COO = 3
    !> Block Sparse Row format
    integer(int32), parameter, public :: MATRIX_BSR = 4

    ! type :: type_constants_matrix_type
    !     type(type_constant_id) :: DENSE = type_constant_id("MATRIX_TYPE", "dense", 1)
    !     type(type_constant_id) :: CSR = type_constant_id("MATRIX_TYPE", "csr", 2)
    !     type(type_constant_id) :: COO = type_constant_id("MATRIX_TYPE", "coo", 3)
    !     type(type_constant_id) :: BSR = type_constant_id("MATRIX_TYPE", "bsr", 4)
    ! contains
    !     procedure, public, pass(self) :: to_id => to_id_matrix_type
    !     procedure, public, pass(self) :: to_name => to_name_matrix_type
    ! end type type_constants_matrix_type

    ! type(type_constants_matrix_type), parameter, public :: MATRIX_TYPES = type_constants_matrix_type()

    ! ==========================================================
    ! Coupling Modes
    ! ==========================================================
    !> Staggered (sequential) coupling scheme
    integer(int32), parameter, public :: COUPLING_MODE_STAGGERED = 1
    !> Monolithic (fully coupled) scheme
    integer(int32), parameter, public :: COUPLING_MODE_MONOLITHIC = 2

    ! ==========================================================
    ! Physics Types
    ! ==========================================================
    !> Thermal analysis identifier
    integer(int32), parameter, public :: PHYSICS_TYPE_THERMAL = 1
    !> Hydraulic analysis identifier
    integer(int32), parameter, public :: PHYSICS_TYPE_HYDRAULIC = 2
    !> Mechanical analysis identifier
    integer(int32), parameter, public :: PHYSICS_TYPE_MECHANICAL = 3
    !> Total number of supported physics types
    integer(int32), parameter, public :: NUM_PHYSICS_TYPES = 3

    ! type :: type_constants_physics_type
    !     type(type_constant_id) :: THERMAL = type_constant_id("PHYSICS_TYPE", "thermal", 1)
    !     type(type_constant_id) :: HYDRAULIC = type_constant_id("PHYSICS_TYPE", "hydraulic", 2)
    !     type(type_constant_id) :: MECHANICAL = type_constant_id("PHYSICS_TYPE", "mechanical", 3)
    !     integer(int32) :: NUM_ID = 3
    ! contains
    !     procedure, public, pass(self) :: to_id => to_id_physics_type
    !     procedure, public, pass(self) :: to_name => to_name_physics_type
    !     procedure, private, pass(self) :: to_object_from_id => to_object_from_id_physics_type
    !     procedure, private, pass(self) :: to_object_from_name => to_object_from_name_physics_type
    !     generic, public :: to_object => to_object_from_id, to_object_from_name
    ! end type type_constants_physics_type

    ! type(type_constants_physics_type), parameter, public :: PHYSICS_TYPES = type_constants_physics_type()

    ! ==========================================================
    ! Degree of Freedom (DOF) Types
    ! ==========================================================
    !> Temperature degree of freedom
    integer(int32), parameter, public :: DOF_TYPE_T = 1
    !> Pressure degree of freedom
    integer(int32), parameter, public :: DOF_TYPE_P = 2
    !> Displacement in the X-direction
    integer(int32), parameter, public :: DOF_TYPE_DX = 3
    !> Displacement in the Y-direction
    integer(int32), parameter, public :: DOF_TYPE_DY = 4
    !> Displacement in the Z-direction
    integer(int32), parameter, public :: DOF_TYPE_DZ = 5

    ! ==========================================================
    ! Thermal Boundary Conditions
    ! ==========================================================
    !> Dirichlet boundary condition (prescribed temperature)
    integer(int32), parameter, public :: THERMAL_BC_DIRICHLET = 101
    !> Neumann boundary condition (prescribed heat flux)
    integer(int32), parameter, public :: THERMAL_BC_NEUMANN = 102
    !> Specific heat flux condition
    integer(int32), parameter, public :: THERMAL_BC_FLUX = 103
    !> Robin boundary condition (mixed)
    integer(int32), parameter, public :: THERMAL_BC_ROBIN = 104
    !> Convective heat transfer condition
    integer(int32), parameter, public :: THERMAL_BC_CONVECTIVE = 105
    !> Radiative heat transfer condition
    integer(int32), parameter, public :: THERMAL_BC_RADIATION = 106
    !> Adiabatic condition (zero heat flux)
    integer(int32), parameter, public :: THERMAL_BC_ADIABATIC = 107
    !> Free surface condition
    integer(int32), parameter, public :: THERMAL_BC_FREE = 108
    !> Total number of thermal boundary condition types
    integer(int32), parameter, public :: NUM_THERMAL_BC_TYPES = 8
    !> Defines the application priority for thermal BCs.
    integer(int32), parameter, public :: THERMAL_BC_SEQUENCE(NUM_THERMAL_BC_TYPES) = [ &
                                         THERMAL_BC_ROBIN, &
                                         THERMAL_BC_CONVECTIVE, &
                                         THERMAL_BC_RADIATION, &
                                         THERMAL_BC_FLUX, &
                                         THERMAL_BC_ADIABATIC, &
                                         THERMAL_BC_FREE, &
                                         THERMAL_BC_NEUMANN, &
                                         THERMAL_BC_DIRICHLET]

    ! ==========================================================
    ! Hydraulic Boundary Conditions
    ! ==========================================================
    !> Dirichlet boundary condition (prescribed pressure/head)
    integer(int32), parameter, public :: HYDRAULIC_BC_DIRICHLET = 201
    !> Neumann boundary condition (prescribed flow rate)
    integer(int32), parameter, public :: HYDRAULIC_BC_NEUMANN = 202
    !> Specific flux condition
    integer(int32), parameter, public :: HYDRAULIC_BC_FLUX = 203
    !> Impermeable condition (zero flow)
    integer(int32), parameter, public :: HYDRAULIC_BC_IMPERMEABLE = 204
    !> Seepage face condition
    integer(int32), parameter, public :: HYDRAULIC_BC_SEEPAGE = 205
    !> Total number of hydraulic boundary condition types
    integer(int32), parameter, public :: NUM_HYDRAULIC_BC_TYPES = 5
    !> Defines the application priority for hydraulic BCs.
    integer(int32), parameter, public :: HYDRAULIC_BC_SEQUENCE(NUM_HYDRAULIC_BC_TYPES) = [ &
                                         HYDRAULIC_BC_FLUX, &
                                         HYDRAULIC_BC_IMPERMEABLE, &
                                         HYDRAULIC_BC_SEEPAGE, &
                                         HYDRAULIC_BC_NEUMANN, &
                                         HYDRAULIC_BC_DIRICHLET]

    ! ==========================================================
    ! Initial Conditions
    ! ==========================================================
    ! --- Target variable for the initial condition ---
    !> Target for thermal initial condition
    integer(int32), parameter, public :: IC_TARGET_THERMAL = 1
    !> Target for hydraulic initial condition
    integer(int32), parameter, public :: IC_TARGET_HYDRAULIC = 2
    !> Target for mechanical initial condition
    integer(int32), parameter, public :: IC_TARGET_MECHANICAL = 3
    !> Target for porosity initial condition
    integer(int32), parameter, public :: IC_TARGET_POROSITY = 4
    !> Total number of initial condition targets
    integer(int32), parameter, public :: NUM_IC_TARGETS = 4
    !> Defines the application priority for initial conditions.
    integer(int32), parameter, public :: IC_TARGET_SEQUENCE(NUM_IC_TARGETS) = [ &
                                         IC_TARGET_THERMAL, &
                                         IC_TARGET_HYDRAULIC, &
                                         IC_TARGET_MECHANICAL, &
                                         IC_TARGET_POROSITY]
    ! --- Method for setting the initial condition ---
    !> Set a uniform value across the domain
    integer(int32), parameter, public :: IC_METHOD_UNIFORM = 1
    !> Interpolate from boundary values using a Laplace equation
    integer(int32), parameter, public :: IC_METHOD_LAPLACE = 2
    !> Read values from an input file
    integer(int32), parameter, public :: IC_METHOD_FROM_FILE = 3
    !> Total number of initial condition methods
    integer(int32), parameter, public :: NUM_IC_METHODS = 3
    integer(int32), parameter, public :: IC_METHOD_SEQUENCE(NUM_IC_METHODS) = [ &
                                         IC_METHOD_UNIFORM, &
                                         IC_METHOD_LAPLACE, &
                                         IC_METHOD_FROM_FILE]

    ! ==========================================================
    ! Nonlinear Solver Settings
    ! ==========================================================
    !> No nonlinear solver is used
    integer(int32), parameter, public :: NONLINEAR_SOLVER_NONE = 0
    !> Newton-Raphson method
    integer(int32), parameter, public :: NONLINEAR_SOLVER_NEWTON = 1
    !> Modified Newton-Raphson method
    integer(int32), parameter, public :: NONLINEAR_SOLVER_MODIFIED_NEWTON = 2
    !> Picard iteration method
    integer(int32), parameter, public :: NONLINEAR_SOLVER_PICARD = 3
    !> Modified Picard iteration method
    integer(int32), parameter, public :: NONLINEAR_SOLVER_MODIFIED_PICARD = 4

    ! type :: type_constants_nonlinear_solver
    !     !> No nonlinear solver is used
    !     type(type_constant_id) :: NONE = type_constant_id("NONLINEAR_SOLVER", "none", 0)
    !     !> Newton-Raphson method
    !     type(type_constant_id) :: NEWTON = type_constant_id("NONLINEAR_SOLVER", "newton", 1)
    !     !> Modified Newton-Raphson method
    !     type(type_constant_id) :: MODIFIED_NEWTON = type_constant_id("NONLINEAR_SOLVER", "modified_newton", 2)
    !     !> Picard iteration method
    !     type(type_constant_id) :: PICARD = type_constant_id("NONLINEAR_SOLVER", "picard", 3)
    !     !> Modified Picard iteration method
    !     type(type_constant_id) :: MODIFIED_PICARD = type_constant_id("NONLINEAR_SOLVER", "modified_picard", 4)
    !     !> Total number of nonlinear solver types
    !     integer(int32) :: NUM_ID = 5
    ! contains
    !     procedure, public, pass(self) :: to_id => to_id_nonlinear_solver
    !     procedure, public, pass(self) :: to_name => to_name_nonlinear_solver
    !     procedure, private, pass(self) :: to_object_from_id => to_object_from_id_nonlinear_solver
    !     procedure, private, pass(self) :: to_object_from_name => to_object_from_name_nonlinear_solver
    !     generic, public :: to_object => to_object_from_id, to_object_from_name
    ! end type type_constants_nonlinear_solver

    ! type(type_constants_nonlinear_solver), parameter, public :: NONLINEAR_SOLVER = type_constants_nonlinear_solver()

    !> Do not check a norm for convergence
    integer(int32), parameter, public :: NONLINEAR_NORM_CRITERIA_NONE = 0
    !> Use the residual vector norm for convergence
    integer(int32), parameter, public :: NONLINEAR_NORM_CRITERIA_RESIDUAL = 1
    !> Use the solution update vector norm for convergence
    integer(int32), parameter, public :: NONLINEAR_NORM_CRITERIA_UPDATE = 2
    !> Use both the residual and update norms for convergence
    integer(int32), parameter, public :: NONLINEAR_NORM_CRITERIA_BOTH = 3

    ! type :: type_constants_nonlinear_norm_criteria
    !     !> Do not check a norm for convergence
    !     type(type_constant_id) :: NONE = type_constant_id("NONLINEAR_NORM_CRITERIA", "none", 0)
    !     !> Use the residual vector norm for convergence
    !     type(type_constant_id) :: RESIDUAL = type_constant_id("NONLINEAR_NORM_CRITERIA", "residual", 1)
    !     !> Use the solution update vector norm for convergence
    !     type(type_constant_id) :: UPDATE = type_constant_id("NONLINEAR_NORM_CRITERIA", "update", 2)
    !     !> Use both the residual and update norms for convergence
    !     type(type_constant_id) :: BOTH = type_constant_id("NONLINEAR_NORM_CRITERIA", "both", 3)
    !     !> Total number of nonlinear norm criteria types
    !     integer(int32) :: NUM_ID = 4
    ! contains
    !     procedure, public, pass(self) :: to_id => to_id_nonlinear_norm_criteria
    !     procedure, public, pass(self) :: to_name => to_name_nonlinear_norm_criteria
    !     procedure, private, pass(self) :: to_object_from_id => to_object_from_id_nonlinear_norm_criteria
    !     procedure, private, pass(self) :: to_object_from_name => to_object_from_name_nonlinear_norm_criteria
    !     generic, public :: to_object => to_object_from_id, to_object_from_name
    ! end type type_constants_nonlinear_norm_criteria

    ! type(type_constants_nonlinear_norm_criteria), parameter, public :: NONLINEAR_NORM_CRITERIA = type_constants_nonlinear_norm_criteria()

    ! type :: type_constants_nonlinear_criteria
    !     !> Do not use a convergence criteria
    !     type(type_constant_id) :: NONE = type_constant_id("NONLINEAR_CRITERIA", "none", 0)
    !     !> Use a relative tolerance for convergence
    !     type(type_constant_id) :: RELATIVE = type_constant_id("NONLINEAR_CRITERIA", "relative", 1)
    !     !> Use an absolute tolerance for convergence
    !     type(type_constant_id) :: ABSOLUTE = type_constant_id("NONLINEAR_CRITERIA", "absolute", 2)
    !     !> Use both relative and absolute tolerances for convergence
    !     type(type_constant_id) :: BOTH = type_constant_id("NONLINEAR_CRITERIA", "both", 3)
    !     !> Total number of nonlinear criteria types
    !     integer(int32) :: NUM_ID = 4
    ! contains
    !     procedure, public, pass(self) :: to_id => to_id_nonlinear_criteria
    !     procedure, public, pass(self) :: to_name => to_name_nonlinear_criteria
    !     procedure, private, pass(self) :: to_object_from_id => to_object_from_id_nonlinear_criteria
    !     procedure, private, pass(self) :: to_object_from_name => to_object_from_name_nonlinear_criteria
    !     generic, public :: to_object => to_object_from_id, to_object_from_name
    ! end type type_constants_nonlinear_criteria

    ! type(type_constants_nonlinear_criteria), parameter, public :: NONLINEAR_CRITERIA = type_constants_nonlinear_criteria()

    !> Do not use a convergence criteria
    integer(int32), parameter, public :: NONLINEAR_CRITERIA_NONE = 0
    !> Use a relative tolerance for convergence
    integer(int32), parameter, public :: NONLINEAR_CRITERIA_RELATIVE = 1
    !> Use an absolute tolerance for convergence
    integer(int32), parameter, public :: NONLINEAR_CRITERIA_ABSOLUTE = 2
    !> Use both relative and absolute tolerances for convergence
    integer(int32), parameter, public :: NONLINEAR_CRITERIA_BOTH = 3

    ! type :: type_constants_nonlinear_logics
    !     !> Logical OR for combining criteria
    !     type(type_constant_id) :: OR = type_constant_id("NONLINEAR_LOGIC", "or", 1)
    !     !> Logical AND for combining criteria
    !     type(type_constant_id) :: AND = type_constant_id("NONLINEAR_LOGIC", "and", 2)
    !     !> Total number of nonlinear logic types
    !     integer(int32) :: NUM_ID = 2
    ! contains
    !     procedure, public, pass(self) :: to_id => to_id_nonlinear_logic
    !     procedure, public, pass(self) :: to_name => to_name_nonlinear_logic
    !     procedure, private, pass(self) :: to_object_from_id => to_object_from_id_nonlinear_logic
    !     procedure, private, pass(self) :: to_object_from_name => to_object_from_name_nonlinear_logic
    !     generic, public :: to_object => to_object_from_id, to_object_from_name
    ! end type type_constants_nonlinear_logics

    ! type(type_constants_nonlinear_logics), parameter, public :: NONLINEAR_LOGICS = type_constants_nonlinear_logics()

    !> Logical OR for combining criteria
    integer(int32), parameter, public :: NONLINEAR_LOGIC_OR = 1
    !> Logical AND for combining criteria
    integer(int32), parameter, public :: NONLINEAR_LOGIC_AND = 2

    ! ==========================================================
    ! Miscellaneous Constants
    ! ==========================================================
    !> L2 (Euclidean) norm
    integer(int32), parameter, public :: NORM_TYPE_L2 = 2
    !> L-infinity (maximum absolute value) norm
    integer(int32), parameter, public :: NORM_TYPE_LINF = 3

    ! type :: type_constants_norm_types
    !     !> L1 (Manhattan) norm
    !     type(type_constant_id) :: L1 = type_constant_id("NORM_TYPE", "l1", 1)
    !     !> L2 (Euclidean) norm
    !     type(type_constant_id) :: L2 = type_constant_id("NORM_TYPE", "l2", 2)
    !     !> L-infinity (maximum absolute value) norm
    !     type(type_constant_id) :: LINF = type_constant_id("NORM_TYPE", "linf", 3)
    !     !> Total number of norm types
    !     integer(int32) :: NUM_ID = 3
    ! contains
    !     procedure, public, pass(self) :: to_id => to_id_norm_types
    !     procedure, public, pass(self) :: to_name => to_name_norm_types
    !     procedure, private, pass(self) :: to_object_from_id => to_object_from_id_norm_types
    !     procedure, private, pass(self) :: to_object_from_name => to_object_from_name_norm_types
    !     generic, public :: to_object => to_object_from_id, to_object_from_name
    ! end type type_constants_norm_types

    ! type(type_constants_norm_types), parameter, public :: NORM_TYPES = type_constants_norm_types()

    type :: type_constants_time_units
        !> Time unit in seconds
        type(type_constant_value) :: SECONDS = type_constant_value("TIME_UNIT", "second", 1, "s", 1.0d0)
        !> Time unit in minutes
        type(type_constant_value) :: MINUTES = type_constant_value("TIME_UNIT", "minute", 2, "m", 60.0d0)
        !> Time unit in hours
        type(type_constant_value) :: HOURS = type_constant_value("TIME_UNIT", "hour", 3, "h", 3600.0d0)
        !> Time unit in days
        type(type_constant_value) :: DAYS = type_constant_value("TIME_UNIT", "day", 4, "d", 86400.0d0)
        !> Time unit in years
        type(type_constant_value) :: YEARS = type_constant_value("TIME_UNIT", "year", 5, "y", 31536000.0d0)
    contains
        procedure, public, pass(self) :: to_id => to_id_time_units
        procedure, public, pass(self) :: to_name => to_name_time_units
        procedure, public, pass(self) :: to_object_from_id => to_object_from_id_time_units
        procedure, public, pass(self) :: to_object_from_name => to_object_from_name_time_units
        generic, public :: to_object => to_object_from_id, to_object_from_name

    end type type_constants_time_units

    type(type_constants_time_units), parameter, public :: TIME_UNITS = type_constants_time_units()

    integer(int32), parameter, public :: TIME_UNIT_SECONDS = 1
    integer(int32), parameter, public :: TIME_UNIT_MINUTES = 2
    integer(int32), parameter, public :: TIME_UNIT_HOURS = 3
    integer(int32), parameter, public :: TIME_UNIT_DAYS = 4
    integer(int32), parameter, public :: TIME_UNIT_YEARS = 5

    !> Time recording point at the start
    integer(int32), parameter, public :: TIME_RECORD_START = 1
    !> Time recording point at the end
    integer(int32), parameter, public :: TIME_RECORD_END = 2

    ! --- Internal constants for spatial dimensions ---
    !> 2D XY dimension
    integer(int32), parameter, public :: COMP_TYPE_2D_XY = 1
    !> 2D XZ dimension
    integer(int32), parameter, public :: COMP_TYPE_2D_XZ = 2
    !> 3D XYZ dimension
    integer(int32), parameter, public :: COMP_TYPE_3D = 3

    ! ==========================================================
    ! Linear Solver Types
    ! ==========================================================
    !> Conjugate Gradient
    integer(int32), parameter, public :: SOLVER_CG           = 1 !&
    !> Bi-Conjugate Gradient
    integer(int32), parameter, public :: SOLVER_BICG         = 2 !&
    !> Conjugate Gradient Squared
    integer(int32), parameter, public :: SOLVER_CGS          = 3 !&
    !> Bi-Conjugate Gradient Stabilized
    integer(int32), parameter, public :: SOLVER_BICGSTAB     = 4 !&
    !> Bi-Conjugate Gradient Stabilized (l)
    integer(int32), parameter, public :: SOLVER_BICGSTAB_L   = 5 !&
    !> Generalized Product-type Bi-Conjugate Gradient
    integer(int32), parameter, public :: SOLVER_GPBICG       = 6 !&
    !> Transpose-Free Quasi-Minimal Residual
    integer(int32), parameter, public :: SOLVER_TFQMR        = 7 !&
    !> Orthomin(m)
    integer(int32), parameter, public :: SOLVER_ORTHOMIN_M   = 8 !&
    !> Generalized Minimal Residual (m)
    integer(int32), parameter, public :: SOLVER_GMRES_M      = 9 !&
    !> Jacobi
    integer(int32), parameter, public :: SOLVER_JACOBI       = 10 !&
    !> Gauss-Seidel
    integer(int32), parameter, public :: SOLVER_GAUSS_SEIDEL = 11 !&
    !> Successive Over-Relaxation
    integer(int32), parameter, public :: SOLVER_SOR          = 12 !&
    !> BiCGSafe
    integer(int32), parameter, public :: SOLVER_BICGSAFE     = 13 !&
    !> Conjugate Residual
    integer(int32), parameter, public :: SOLVER_CR           = 14 !&
    !> Bi-Conjugate Residual
    integer(int32), parameter, public :: SOLVER_BICR         = 15 !&
    !> Conjugate Residual Squared
    integer(int32), parameter, public :: SOLVER_CRS          = 16 !&
    !> Bi-Conjugate Residual Stabilized
    integer(int32), parameter, public :: SOLVER_BICRSTAB     = 17 !&
    !> Generalized Product-type Bi-Conjugate Residual
    integer(int32), parameter, public :: SOLVER_GPBICR       = 18 !&
    !> BiCRSafe
    integer(int32), parameter, public :: SOLVER_BICRSAFE     = 19 !&
    !> Flexible GMRES (m)
    integer(int32), parameter, public :: SOLVER_FGMRES_M     = 20 !&
    !> Induced Dimension Reduction (s)
    integer(int32), parameter, public :: SOLVER_IDR_S        = 21 !&
    !> IDR(1)
    integer(int32), parameter, public :: SOLVER_IDR1         = 22 !&
    !> Minimal Residual
    integer(int32), parameter, public :: SOLVER_MINRES       = 23 !&
    !> Conjugate Orthogonal Conjugate Gradient
    integer(int32), parameter, public :: SOLVER_COCG         = 24 !&
    !> Conjugate Orthogonal Conjugate Residual
    integer(int32), parameter, public :: SOLVER_COCR         = 25 !&

    ! ==========================================================
    ! Preconditioner Types
    ! ==========================================================
    !> No preconditioner
    integer(int32), parameter, public :: SOLVER_PRECONDITION_NONE   = 0 !&
    !> Jacobi preconditioner
    integer(int32), parameter, public :: SOLVER_PRECONDITION_JACOBI = 1 !&
    !> ILU(k) (Incomplete LU)
    integer(int32), parameter, public :: SOLVER_PRECONDITION_ILU    = 2 !&
    !> SSOR (Symmetric Successive Over-Relaxation)
    integer(int32), parameter, public :: SOLVER_PRECONDITION_SSOR   = 3 !&
    !> Hybrid preconditioner
    integer(int32), parameter, public :: SOLVER_PRECONDITION_HYBRID = 4 !&
    !> I+S preconditioner
    integer(int32), parameter, public :: SOLVER_PRECONDITION_IS     = 5 !&
    !> SAINV (Sparse Approximate Inverse)
    integer(int32), parameter, public :: SOLVER_PRECONDITION_SAINV  = 6 !&
    !> SA-AMG (Sparse Approximate Algebraic Multigrid)
    integer(int32), parameter, public :: SOLVER_PRECONDITION_SAAMG  = 7 !&
    !> Crout ILU
    integer(int32), parameter, public :: SOLVER_PRECONDITION_ILUC   = 8 !&
    !> ILUT (Incomplete LU with threshold)
    integer(int32), parameter, public :: SOLVER_PRECONDITION_ILUT   = 9 !&

    ! ! ==========================================================
    ! ! Solver Status Codes
    ! ! ==========================================================
    ! type :: type_solver_status
    !     !> Solver completed successfully
    !     type(type_constant_id) :: SUCCESS = type_constant_id("SOLVER_STATUS", "SUCCESS", 0)
    !     !> Solver encountered ill-conditioned options
    !     type(type_constant_id) :: ILL_OPTIONS = type_constant_id("SOLVER_STATUS", "ILL_OPTIONS", -1)
    !     !> Solver encountered a breakdown
    !     type(type_constant_id) :: BREAKDOWN = type_constant_id("SOLVER_STATUS", "BREAKDOWN", -2)
    !     !> Solver ran out of memory
    !     type(type_constant_id) :: OUT_OF_MEMORY = type_constant_id("SOLVER_STATUS", "OUT_OF_MEMORY", -3)
    !     !> Solver reached maximum iterations without convergence
    !     type(type_constant_id) :: MAXITER = type_constant_id("SOLVER_STATUS", "MAXITER", -4)
    !     !> Preconditioner setup failure
    !     type(type_constant_id) :: DECOMPOSITION_FAILURE = type_constant_id("SOLVER_STATUS", "DECOMPOSITION_FAILURE", -5)
    !     !> Solver method not implemented
    !     type(type_constant_id) :: NOT_IMPLEMENTED = type_constant_id("SOLVER_STATUS", "NOT_IMPLEMENTED", -6)
    ! end type type_solver_status

    ! type(type_solver_status), parameter, public :: SOLVER_STATUS = type_solver_status()

    integer(int32), parameter, public :: SOLVER_STATUS_SUCCESS = 0
    integer(int32), parameter, public :: SOLVER_STATUS_ILL_OPTIONS = -1
    integer(int32), parameter, public :: SOLVER_STATUS_BREAKDOWN = -2
    integer(int32), parameter, public :: SOLVER_STATUS_OUT_OF_MEMORY = -3
    integer(int32), parameter, public :: SOLVER_STATUS_MAXITER = -4
    integer(int32), parameter, public :: SOLVER_STATUS_DECOMPOSITION_FAILURE = -5
    integer(int32), parameter, public :: SOLVER_STATUS_NOT_IMPLEMENTED = -6

    ! =========================================================
    ! Matrix Operations
    ! ==========================================================
    !> Insert operation
    integer(int32), parameter, public :: OP_INS = 1
    !> Add operation
    integer(int32), parameter, public :: OP_ADD = 2

    integer(int32), parameter, public :: OP_SCALE_SYMM_DIAG = 3
    integer(int32), parameter, public :: OP_SCALE_JACOBI = 4

    !=========================================================
    ! Matrix Check Status Codes
    !=========================================================
    !> Matrix is valid
    integer(int32), parameter, public :: MATRIX_STATUS_SUCCESS = 0
    !> Matrix has an error
    integer(int32), parameter, public :: MATRIX_STATUS_ILL_OPERATIONS = -1
    integer(int32), parameter, public :: MATRIX_STATUS_OUT_OF_MEMORY = -2
    integer(int32), parameter, public :: MATRIX_STATUS_NOT_INITIALIZED = -3

    !> Matrix method not implemented
    integer(int32), parameter, public :: MATRIX_STATUS_NOT_IMPLEMENTED = -5

    integer(int32), parameter, public :: VECTOR_STATUS_SUCCESS = 0
    integer(int32), parameter, public :: VECTOR_STATUS_ILL_OPERATIONS = -1
    integer(int32), parameter, public :: VECTOR_STATUS_OUT_OF_MEMORY = -3
    integer(int32), parameter, public :: VECTOR_STATUS_NOT_IMPLEMENTED = -5

contains
!     !> Convert matrix type constant to its ID.
!     pure subroutine to_id_matrix_type(self, name, id)
!         implicit none
!         class(type_constants_matrix_type), intent(in) :: self
!         character(len=*), intent(in) :: name
!         integer(int32), intent(inout) :: id

!         character(len=:), allocatable :: lname
!         lname = strip(to_lower(name))

!         if (lname == strip(self%DENSE%name)) then
!             id = self%DENSE%id
!         else if (lname == strip(self%CSR%name)) then
!             id = self%CSR%id
!         else if (lname == strip(self%COO%name)) then
!             id = self%COO%id
!         else if (lname == strip(self%BSR%name)) then
!             id = self%BSR%id
!         else
!             id = -1
!         end if
!     end subroutine to_id_matrix_type

!     !> Convert matrix type ID to its name.
!     pure subroutine to_name_matrix_type(self, id, name)
!         implicit none
!         class(type_constants_matrix_type), intent(in) :: self
!         integer(int32), intent(in) :: id
!         character(len=*), intent(inout) :: name

!         if (id == self%DENSE%id) then
!             name = self%DENSE%name
!         else if (id == self%CSR%id) then
!             name = self%CSR%name
!         else if (id == self%COO%id) then
!             name = self%COO%name
!         else if (id == self%BSR%id) then
!             name = self%BSR%name
!         else
!             name = "unknown"
!         end if
!     end subroutine to_name_matrix_type

!     pure function to_id_physics_type(self, name) result(id)
!         implicit none
!         class(type_constants_physics_type), intent(in) :: self
!         character(len=*), intent(in) :: name
!         integer(int32) :: id

!         character(len=:), allocatable :: lname
!         lname = strip(to_lower(name))

!         if (lname == strip(to_lower(self%THERMAL%name))) then
!             id = self%THERMAL%id
!         else if (lname == strip(to_lower(self%HYDRAULIC%name))) then
!             id = self%HYDRAULIC%id
!         else if (lname == strip(to_lower(self%MECHANICAL%name))) then
!             id = self%MECHANICAL%id
!         else
!             id = -1
!         end if
!     end function to_id_physics_type

!     pure function to_name_physics_type(self, id) result(name)
!         implicit none
!         class(type_constants_physics_type), intent(in) :: self
!         integer(int32), intent(in) :: id
!         character(len=:), allocatable :: name

!         if (id == self%THERMAL%id) then
!             name = self%THERMAL%name
!         else if (id == self%HYDRAULIC%id) then
!             name = self%HYDRAULIC%name
!         else if (id == self%MECHANICAL%id) then
!             name = self%MECHANICAL%name
!         else
!             name = "unknown"
!         end if
!     end function to_name_physics_type

!     pure function to_object_from_id_physics_type(self, id) result(object)
!         implicit none
!         class(type_constants_physics_type), intent(in) :: self
!         integer(int32), intent(in) :: id
!         type(type_constant_id) :: object

!         if (id == self%THERMAL%id) then
!             object = self%THERMAL
!         else if (id == self%HYDRAULIC%id) then
!             object = self%HYDRAULIC
!         else if (id == self%MECHANICAL%id) then
!             object = self%MECHANICAL
!         else
!             object = type_constant_id("PHYSICS_TYPE", "unknown", -1)
!         end if

!     end function to_object_from_id_physics_type

!     pure function to_object_from_name_physics_type(self, name) result(object)
!         implicit none
!         class(type_constants_physics_type), intent(in) :: self
!         character(len=*), intent(in) :: name
!         type(type_constant_id) :: object

!         character(len=:), allocatable :: lname
!         lname = strip(to_lower(name))

!         if (lname == strip(to_lower(self%THERMAL%name))) then
!             object = self%THERMAL
!         else if (lname == strip(to_lower(self%HYDRAULIC%name))) then
!             object = self%HYDRAULIC
!         else if (lname == strip(to_lower(self%MECHANICAL%name))) then
!             object = self%MECHANICAL
!         else
!             object = type_constant_id("PHYSICS_TYPE", "unknown", -1)
!         end if
!     end function to_object_from_name_physics_type

    !> Convert time unit constant to its ID.
    pure function to_id_time_units(self, name) result(id)
        implicit none
        class(type_constants_time_units), intent(in) :: self
        character(len=*), intent(in) :: name
        integer(int32) :: id

        character(len=:), allocatable :: lname
        lname = strip(to_lower(name))

        if (lname == strip(to_lower(self%SECONDS%name))) then
            id = self%SECONDS%id
        else if (lname == strip(to_lower(self%MINUTES%name))) then
            id = self%MINUTES%id
        else if (lname == strip(to_lower(self%HOURS%name))) then
            id = self%HOURS%id
        else if (lname == strip(to_lower(self%DAYS%name))) then
            id = self%DAYS%id
        else if (lname == strip(to_lower(self%YEARS%name))) then
            id = self%YEARS%id
        else
            id = -1
        end if
    end function to_id_time_units

    !> Convert time unit ID to its name.
    pure function to_name_time_units(self, id) result(name)
        implicit none
        class(type_constants_time_units), intent(in) :: self
        integer(int32), intent(in) :: id
        character(len=:), allocatable :: name

        if (id == self%SECONDS%id) then
            name = self%SECONDS%name
        else if (id == self%MINUTES%id) then
            name = self%MINUTES%name
        else if (id == self%HOURS%id) then
            name = self%HOURS%name
        else if (id == self%DAYS%id) then
            name = self%DAYS%name
        else if (id == self%YEARS%id) then
            name = self%YEARS%name
        else
            name = "unknown"
        end if
    end function to_name_time_units

    !> Convert time unit ID to its object.
    pure function to_object_from_id_time_units(self, id) result(object)
        implicit none
        class(type_constants_time_units), intent(in) :: self
        integer(int32), intent(in) :: id
        type(type_constant_value) :: object

        if (id == self%SECONDS%id) then
            object = self%SECONDS
        else if (id == self%MINUTES%id) then
            object = self%MINUTES
        else if (id == self%HOURS%id) then
            object = self%HOURS
        else if (id == self%DAYS%id) then
            object = self%DAYS
        else if (id == self%YEARS%id) then
            object = self%YEARS
        else
            object = type_constant_value("TIME_UNIT", "unknown", -1, "unknown", 0.0d0)
        end if

    end function to_object_from_id_time_units

    !> Convert time unit name to its object.
    pure function to_object_from_name_time_units(self, name) result(object)
        implicit none
        class(type_constants_time_units), intent(in) :: self
        character(len=*), intent(in) :: name
        type(type_constant_value) :: object

        character(len=:), allocatable :: lname
        lname = strip(to_lower(name))

        if (lname == strip(to_lower(self%SECONDS%name))) then
            object = self%SECONDS
        else if (lname == strip(to_lower(self%MINUTES%name))) then
            object = self%MINUTES
        else if (lname == strip(to_lower(self%HOURS%name))) then
            object = self%HOURS
        else if (lname == strip(to_lower(self%DAYS%name))) then
            object = self%DAYS
        else if (lname == strip(to_lower(self%YEARS%name))) then
            object = self%YEARS
        else
            object = type_constant_value("TIME_UNIT", "unknown", -1, "unknown", 0.0d0)
        end if
    end function to_object_from_name_time_units

!     pure function to_id_nonlinear_criteria(self, name) result(id)
!         implicit none
!         class(type_constants_nonlinear_criteria), intent(in) :: self
!         character(len=*), intent(in) :: name
!         integer(int32) :: id

!         character(len=:), allocatable :: lname
!         lname = strip(to_lower(name))

!         if (lname == strip(to_lower(self%none%name))) then
!             id = self%none%id
!         else if (lname == strip(to_lower(self%RELATIVE%name))) then
!             id = self%RELATIVE%id
!         else if (lname == strip(to_lower(self%ABSOLUTE%name))) then
!             id = self%ABSOLUTE%id
!         else if (lname == strip(to_lower(self%BOTH%name))) then
!             id = self%BOTH%id
!         else
!             id = -1
!         end if
!     end function to_id_nonlinear_criteria

!     pure function to_name_nonlinear_criteria(self, id) result(name)
!         implicit none
!         class(type_constants_nonlinear_criteria), intent(in) :: self
!         integer(int32), intent(in) :: id
!         character(len=:), allocatable :: name

!         if (id == self%none%id) then
!             name = self%none%name
!         else if (id == self%RELATIVE%id) then
!             name = self%RELATIVE%name
!         else if (id == self%ABSOLUTE%id) then
!             name = self%ABSOLUTE%name
!         else if (id == self%BOTH%id) then
!             name = self%BOTH%name
!         else
!             name = "unknown"
!         end if
!     end function to_name_nonlinear_criteria

!     pure function to_id_nonlinear_logic(self, name) result(id)
!         implicit none
!         class(type_constants_nonlinear_logics), intent(in) :: self
!         character(len=*), intent(in) :: name
!         integer(int32) :: id

!         character(len=:), allocatable :: lname
!         lname = strip(to_lower(name))

!         if (lname == strip(to_lower(self%OR%name))) then
!             id = self%OR%id
!         else if (lname == strip(to_lower(self%AND%name))) then
!             id = self%AND%id
!         else
!             id = -1
!         end if
!     end function to_id_nonlinear_logic

!     pure function to_name_nonlinear_logic(self, id) result(name)
!         implicit none
!         class(type_constants_nonlinear_logics), intent(in) :: self
!         integer(int32), intent(in) :: id
!         character(len=:), allocatable :: name

!         if (id == self%OR%id) then
!             name = self%OR%name
!         else if (id == self%AND%id) then
!             name = self%AND%name
!         else
!             name = "unknown"
!         end if
!     end function to_name_nonlinear_logic

!     pure function to_object_from_id_nonlinear_logic(self, id) result(object)
!         implicit none
!         class(type_constants_nonlinear_logics), intent(in) :: self
!         integer(int32), intent(in) :: id
!         type(type_constant_id) :: object

!         if (id == self%OR%id) then
!             object = self%OR
!         else if (id == self%AND%id) then
!             object = self%AND
!         else
!             object = type_constant_id("NONLINEAR_LOGIC", "unknown", -1)
!         end if

!     end function to_object_from_id_nonlinear_logic

!     pure function to_object_from_name_nonlinear_logic(self, name) result(object)
!         implicit none
!         class(type_constants_nonlinear_logics), intent(in) :: self
!         character(len=*), intent(in) :: name
!         type(type_constant_id) :: object

!         character(len=:), allocatable :: lname
!         lname = strip(to_lower(name))

!         if (lname == strip(to_lower(self%OR%name))) then
!             object = self%OR
!         else if (lname == strip(to_lower(self%AND%name))) then
!             object = self%AND
!         else
!             object = type_constant_id("NONLINEAR_LOGIC", "unknown", -1)
!         end if
!     end function to_object_from_name_nonlinear_logic

!     pure function to_object_from_id_nonlinear_criteria(self, id) result(object)
!         implicit none
!         class(type_constants_nonlinear_criteria), intent(in) :: self
!         integer(int32), intent(in) :: id
!         type(type_constant_id) :: object

!         if (id == self%none%id) then
!             object = self%none
!         else if (id == self%RELATIVE%id) then
!             object = self%RELATIVE
!         else if (id == self%ABSOLUTE%id) then
!             object = self%ABSOLUTE
!         else if (id == self%BOTH%id) then
!             object = self%BOTH
!         else
!             object = type_constant_id("NONLINEAR_CRITERIA", "unknown", -1)
!         end if

!     end function to_object_from_id_nonlinear_criteria

!     pure function to_object_from_name_nonlinear_criteria(self, name) result(object)
!         implicit none
!         class(type_constants_nonlinear_criteria), intent(in) :: self
!         character(len=*), intent(in) :: name
!         type(type_constant_id) :: object

!         character(len=:), allocatable :: lname
!         lname = strip(to_lower(name))

!         if (lname == strip(to_lower(self%none%name))) then
!             object = self%none
!         else if (lname == strip(to_lower(self%RELATIVE%name))) then
!             object = self%RELATIVE
!         else if (lname == strip(to_lower(self%ABSOLUTE%name))) then
!             object = self%ABSOLUTE
!         else if (lname == strip(to_lower(self%BOTH%name))) then
!             object = self%BOTH
!         else
!             object = type_constant_id("NONLINEAR_CRITERIA", "unknown", -1)
!         end if
!     end function to_object_from_name_nonlinear_criteria

!     pure function to_id_norm_types(self, name) result(id)
!         implicit none
!         class(type_constants_norm_types), intent(in) :: self
!         character(len=*), intent(in) :: name
!         integer(int32) :: id

!         character(len=:), allocatable :: lname
!         lname = strip(to_lower(name))

!         if (lname == strip(to_lower(self%L1%name))) then
!             id = self%L1%id
!         else if (lname == strip(to_lower(self%L2%name))) then
!             id = self%L2%id
!         else if (lname == strip(to_lower(self%LINF%name))) then
!             id = self%LINF%id
!         else
!             id = -1
!         end if
!     end function to_id_norm_types

!     pure function to_name_norm_types(self, id) result(name)
!         implicit none
!         class(type_constants_norm_types), intent(in) :: self
!         integer(int32), intent(in) :: id
!         character(len=:), allocatable :: name

!         if (id == self%L1%id) then
!             name = self%L1%name
!         else if (id == self%L2%id) then
!             name = self%L2%name
!         else if (id == self%LINF%id) then
!             name = self%LINF%name
!         else
!             name = "unknown"
!         end if
!     end function to_name_norm_types

!     pure function to_object_from_id_norm_types(self, id) result(object)
!         implicit none
!         class(type_constants_norm_types), intent(in) :: self
!         integer(int32), intent(in) :: id
!         type(type_constant_id) :: object

!         if (id == self%L1%id) then
!             object = self%L1
!         else if (id == self%L2%id) then
!             object = self%L2
!         else if (id == self%LINF%id) then
!             object = self%LINF
!         else
!             object = type_constant_id("NORM_TYPE", "unknown", -1)
!         end if

!     end function to_object_from_id_norm_types

!     pure function to_object_from_name_norm_types(self, name) result(object)
!         implicit none
!         class(type_constants_norm_types), intent(in) :: self
!         character(len=*), intent(in) :: name
!         type(type_constant_id) :: object

!         character(len=:), allocatable :: lname
!         lname = strip(to_lower(name))

!         if (lname == strip(to_lower(self%L1%name))) then
!             object = self%L1
!         else if (lname == strip(to_lower(self%L2%name))) then
!             object = self%L2
!         else if (lname == strip(to_lower(self%LINF%name))) then
!             object = self%LINF
!         else
!             object = type_constant_id("NORM_TYPE", "unknown", -1)
!         end if
!     end function to_object_from_name_norm_types

!     pure function to_id_nonlinear_norm_criteria(self, name) result(id)
!         implicit none
!         class(type_constants_nonlinear_norm_criteria), intent(in) :: self
!         character(len=*), intent(in) :: name
!         integer(int32) :: id

!         character(len=:), allocatable :: lname
!         lname = strip(to_lower(name))

!         if (lname == strip(to_lower(self%none%name))) then
!             id = self%none%id
!         else if (lname == strip(to_lower(self%RESIDUAL%name))) then
!             id = self%RESIDUAL%id
!         else if (lname == strip(to_lower(self%UPDATE%name))) then
!             id = self%UPDATE%id
!         else if (lname == strip(to_lower(self%BOTH%name))) then
!             id = self%BOTH%id
!         else
!             id = -1
!         end if
!     end function to_id_nonlinear_norm_criteria

!     pure function to_name_nonlinear_norm_criteria(self, id) result(name)
!         implicit none
!         class(type_constants_nonlinear_norm_criteria), intent(in) :: self
!         integer(int32), intent(in) :: id
!         character(len=:), allocatable :: name

!         if (id == self%none%id) then
!             name = self%none%name
!         else if (id == self%RESIDUAL%id) then
!             name = self%RESIDUAL%name
!         else if (id == self%UPDATE%id) then
!             name = self%UPDATE%name
!         else if (id == self%BOTH%id) then
!             name = self%BOTH%name
!         else
!             name = "unknown"
!         end if
!     end function to_name_nonlinear_norm_criteria

!     pure function to_object_from_id_nonlinear_norm_criteria(self, id) result(object)
!         implicit none
!         class(type_constants_nonlinear_norm_criteria), intent(in) :: self
!         integer(int32), intent(in) :: id
!         type(type_constant_id) :: object

!         if (id == self%none%id) then
!             object = self%none
!         else if (id == self%RESIDUAL%id) then
!             object = self%RESIDUAL
!         else if (id == self%UPDATE%id) then
!             object = self%UPDATE
!         else if (id == self%BOTH%id) then
!             object = self%BOTH
!         else
!             object = type_constant_id("NONLINEAR_NORM_CRITERIA", "unknown", -1)
!         end if

!     end function to_object_from_id_nonlinear_norm_criteria

!     pure function to_object_from_name_nonlinear_norm_criteria(self, name) result(object)
!         implicit none
!         class(type_constants_nonlinear_norm_criteria), intent(in) :: self
!         character(len=*), intent(in) :: name
!         type(type_constant_id) :: object

!         character(len=:), allocatable :: lname
!         lname = strip(to_lower(name))

!         if (lname == strip(to_lower(self%none%name))) then
!             object = self%none
!         else if (lname == strip(to_lower(self%RESIDUAL%name))) then
!             object = self%RESIDUAL
!         else if (lname == strip(to_lower(self%UPDATE%name))) then
!             object = self%UPDATE
!         else if (lname == strip(to_lower(self%BOTH%name))) then
!             object = self%BOTH
!         else
!             object = type_constant_id("NONLINEAR_NORM_CRITERIA", "unknown", -1)
!         end if
!     end function to_object_from_name_nonlinear_norm_criteria

!     pure function to_id_nonlinear_solver(self, name) result(id)
!         implicit none
!         class(type_constants_nonlinear_solver), intent(in) :: self
!         character(len=*), intent(in) :: name
!         integer(int32) :: id

!         character(len=:), allocatable :: lname
!         lname = strip(to_lower(name))

!         if (lname == strip(to_lower(self%NEWTON%name))) then
!             id = self%NEWTON%id
!         else if (lname == strip(to_lower(self%MODIFIED_NEWTON%name))) then
!             id = self%MODIFIED_NEWTON%id
!         else if (lname == strip(to_lower(self%PICARD%name))) then
!             id = self%PICARD%id
!         else if (lname == strip(to_lower(self%MODIFIED_PICARD%name))) then
!             id = self%MODIFIED_PICARD%id
!         else
!             id = -1
!         end if
!     end function to_id_nonlinear_solver

!     pure function to_name_nonlinear_solver(self, id) result(name)
!         implicit none
!         class(type_constants_nonlinear_solver), intent(in) :: self
!         integer(int32), intent(in) :: id
!         character(len=:), allocatable :: name

!         if (id == self%NEWTON%id) then
!             name = self%NEWTON%name
!         else if (id == self%MODIFIED_NEWTON%id) then
!             name = self%MODIFIED_NEWTON%name
!         else if (id == self%PICARD%id) then
!             name = self%PICARD%name
!         else if (id == self%MODIFIED_PICARD%id) then
!             name = self%MODIFIED_PICARD%name
!         else
!             name = "unknown"
!         end if
!     end function to_name_nonlinear_solver

!     pure function to_object_from_id_nonlinear_solver(self, id) result(object)
!         implicit none
!         class(type_constants_nonlinear_solver), intent(in) :: self
!         integer(int32), intent(in) :: id
!         type(type_constant_id) :: object

!         if (id == self%NEWTON%id) then
!             object = self%NEWTON
!         else if (id == self%MODIFIED_NEWTON%id) then
!             object = self%MODIFIED_NEWTON
!         else if (id == self%PICARD%id) then
!             object = self%PICARD
!         else if (id == self%MODIFIED_PICARD%id) then
!             object = self%MODIFIED_PICARD
!         else
!             object = type_constant_id("NONLINEAR_SOLVER_TYPE", "unknown", -1)
!         end if

!     end function to_object_from_id_nonlinear_solver

!     pure function to_object_from_name_nonlinear_solver(self, name) result(object)
!         implicit none
!         class(type_constants_nonlinear_solver), intent(in) :: self
!         character(len=*), intent(in) :: name
!         type(type_constant_id) :: object

!         character(len=:), allocatable :: lname
!         lname = strip(to_lower(name))

!         if (lname == strip(to_lower(self%NEWTON%name))) then
!             object = self%NEWTON
!         else if (lname == strip(to_lower(self%MODIFIED_NEWTON%name))) then
!             object = self%MODIFIED_NEWTON
!         else if (lname == strip(to_lower(self%PICARD%name))) then
!             object = self%PICARD
!         else if (lname == strip(to_lower(self%MODIFIED_PICARD%name))) then
!             object = self%MODIFIED_PICARD
!         else
!             object = type_constant_id("NONLINEAR_SOLVER_TYPE", "unknown", -1)
!         end if
!     end function to_object_from_name_nonlinear_solver

end module core_constants_solver
