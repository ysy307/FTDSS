module core_types_variable
    ...
    !> 変数管理クラス
    !> (Time, Space) レイアウトによる履歴アクセスの最適化版
    type :: type_variable
        private
        ...
        !> リングバッファのヘッド位置（現在の時刻 t_{n+1} を指すインデックス）
        integer(int32) :: head_idx = 1

        !> メインデータ領域
        !> Dim 1: Time (step+2) -> メモリ上で連続（履歴アクセス最速）
        !> Dim 2: Node (dofs)
        real(real64), allocatable :: values(:, :)

        !> 修正量 delta (Newton update) - ソルバー用に1次元で確保
    ...
    contains
        ...
        ! Setters
        procedure, public, pass(self) :: set_current_array_variable
        procedure, public, pass(self) :: set_current_scalar_variable
        procedure, public, pass(self) :: set_current_scalar_all_variable
        ...
        procedure, public, pass(self) :: get_history_ptr ! 履歴列への連続ポインタ
        procedure, public, pass(self) :: get_current_ptr ! 現在値行へのポインタ
        procedure, public, pass(self) :: get_previous_ptr
        procedure, public, pass(self) :: get_delta_ptr
        procedure, public, pass(self) :: get_diff_ptr

        ! Getters (Scalar)
        procedure, public, pass(self) :: get_current_scalar
        procedure, public, pass(self) :: get_current_gradient_ptr
...
contains
    ...
    subroutine advance_time_step_variable(self)
        ...
        ! 新しいステップの初期値として、前のステップの値をコピー（予測子）
        ! 第2次元へのアクセスなのでストライドコピーになるが、計算全体のコストから見れば微小
    ...
    !> [重要] 履歴データ列へのポインタを取得
    !> 戻り値はメモリ連続な配列になるため、物理計算ループ内で最速です。
    subroutine get_history_ptr(self, node_id, ptr, current_head)
        implicit none
        class(type_variable), intent(in), target :: self
        integer(int32), intent(in) :: node_id

        ! CONTIGUOUS属性をつけられる（最速）
    ...
    end subroutine get_history_ptr

    !> 現在値ベクトルへのポインタを取得
    !> 注意: ストライドアクセスになるため contiguous は付けられません
    subroutine get_current_ptr(self, ptr)
        implicit none
        class(type_variable), intent(in), target :: self
        real(real64), pointer, intent(inout) :: ptr(:) ! NOT contiguous

        if (self%is_initialized) then
            ptr => self%values(self%head_idx, :)
        else
            nullify (ptr)
        end if
    end subroutine get_current_ptr

    !> 1ステップ前(t_n)へのポインタを取得
    subroutine get_previous_ptr(self, ptr)
        implicit none
        class(type_variable), intent(in), target :: self
    ...
    end subroutine get_previous_ptr

    subroutine get_delta_ptr(self, ptr)
        implicit none
        class(type_variable), intent(in), target :: self
        real(real64), pointer, intent(inout), contiguous :: ptr(:)
        if (self%is_initialized) then
    ...
    end subroutine get_delta_ptr

    subroutine get_diff_ptr(self, ptr)
        implicit none
        class(type_variable), intent(in), target :: self
        real(real64), pointer, intent(inout), contiguous :: ptr(:)
        if (self%is_initialized) then
    ...
    end subroutine get_diff_ptr

    subroutine get_current_gradient_ptr(self, ptr)
        implicit none
        class(type_variable), intent(in), target :: self
        type(type_coordinate_array_dp), pointer, intent(inout) :: ptr
        ptr => self%grad
    end subroutine get_current_gradient_ptr

    ! ------------------------------------------------------------------
    ! Scalar Access / Helpers
    ! ------------------------------------------------------------------
    pure subroutine get_current_scalar(self, node_id, val)
        implicit none
    ...
    subroutine compute_time_derivative_variable(self, bdf_coeffs, bdf_order)
        ...
        do i = 1, bdf_order + 1
            ! 行アクセス(ストライド)だが、BLASレベル1相当なので十分高速
