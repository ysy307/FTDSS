module types_discretization_dof_map
    ...
    type :: type_dof_map
        ...
        integer(int32), private :: num_dof_per_node = 0
        !> Number of DOFs for each individual physics type.
        integer(int32), private :: num_dof_of_physics(PHYSICS_TYPES%NUM_ID) = 0
    ...
    contains
        ...
        ! ---- Getter ----
        procedure, public, pass(self) :: get_num_dof_per_node => get_num_dof_per_node_dof_map
        procedure, public, pass(self) :: get_num_dof_of_physics => get_num_dof_of_physics_dof_map
...
contains
    ...
    subroutine destroy_type_dof_map(self)
        ...
        self%num_dof_of_physics = 0
        self%start_dof_index = 0
    end subroutine destroy_type_dof_map

    subroutine get_num_dof_per_node_dof_map(self, num_dof_per_node)
        implicit none
        class(type_dof_map), intent(in) :: self
        integer(int32), intent(inout) :: num_dof_per_node

        num_dof_per_node = self%num_dof_per_node
    end subroutine get_num_dof_per_node_dof_map

    subroutine get_num_dof_of_physics_dof_map(self, physics_id, num_dof)
        implicit none
        class(type_dof_map), intent(in) :: self
        ...
        num_dof = self%num_dof_of_physics(physics_id%ID)

    end subroutine get_num_dof_of_physics_dof_map
