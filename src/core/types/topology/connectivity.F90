module core_types_topology_connectivity
    ...
    !> This structure is used to represent the connectivity of nodes in a mesh, where each element node set is stored
    !> in a compact format for efficient access. The `row_ptr` array indicates the starting index of each element node set
    !> in the `col_ind` array, which contains the actual node IDs for all elements concatenated together.
    !>
    type :: type_csr_index
        !> Index array for CSR format. Stores the starting position of each element node set.
