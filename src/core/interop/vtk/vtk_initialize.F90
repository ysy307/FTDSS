...
contains
    module subroutine type_vtk_vtk_initialize(self, file_name, global_node_id_key, node_type_key, num_sharing_ranks_key, &
        ...
        if (.not. c_associated(self%handle) .or. ierr /= 0) then
            call global_logger%log_error("C++ VTK Reader failed to initialize for file: "//trim(c_file_name))
        ...
        if (self%num_total_cells > 0) then
            ...
            if (present(rank_key)) then
                ...
                call vtk_get_cell_data_int32(self%handle, c_array_name, raw_ranks)
            end if
            if (present(original_id_key)) then
                ...
                call vtk_get_cell_data_int32(self%handle, c_array_name, raw_original_ids)
            ...
            if (present(color_key)) then
                ...
                call vtk_get_cell_data_int32(self%handle, c_array_name, raw_colors)
            end if
...
end submodule core_vtk_vtk_initialize
