...
contains
    module subroutine ElementHolder_allocate(self, iShape_Type, iElem, Global_Coordinate, Connectivity)
        ...
        case (SHAPE_TRIANGLE)
            self%e = TriangleFirst(iElem, Global_Coordinate, Connectivity)
        case (SHAPE_PIXEL)
            self%e = SquareFirst(iElem, Global_Coordinate, Connectivity)
        case (SHAPE_QUAD)
            self%e = SquareFirst(iElem, Global_Coordinate, Connectivity)
        case (SHAPE_QUADRATIC_TRIANGLE)
            self%e = TriangleSecond(iElem, Global_Coordinate, Connectivity)
        case (SHAPE_QUADRATIC_QUAD)
            self%e = SquareSecond(iElem, Global_Coordinate, Connectivity)
        end select
